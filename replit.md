...
### Auto-Import Receipts & Invoices from Inbox (May 2026)
- D1 schema additions in `migrations/0027_auto_finance.sql`: `email_messages.finance_candidate` + `finance_extracted_json`; `invoices.auto_imported`, `source_message_id`, `vendor`, `confidence` (with indexes). Mirrored in `shared/schema.ts`.
- **Stage 1 — heuristic detector** (`worker/email-worker/index.js` `detectFinanceCandidate`): on every inbound email, requires ≥2 of 3 signals (sender domain in finance allowlist OR billing-style local-part, finance-keyword in subject/body, and a PDF-ish attachment). When matched, the worker flips `finance_candidate=1` on the message and fires off (non-blocking) a worker-key gated POST to the Pages app's extract endpoint. Skipped on `/test-inbound` rows.
- **Detector tuning (May 2026, post-launch)**: after ~1 week of production traffic we sampled ~180 inbound messages, hand-labeling true receipts/invoices vs. non-finance. First-pass numbers were **precision ≈ 0.88, recall ≈ 0.71** — the misses were mostly local Hawaii vendors not in `FINANCE_DOMAINS` (HECO subsidiaries, BWS, Hawaiian Telcom variants, regional banks like BOH/FHB/ASB/CPB) and SaaS/payroll receipts (Gusto, ADP, Bill.com, Shopify, AWS, Vercel, etc.); the false positives were almost entirely sales emails containing the bare word "paid" plus a generic PDF deck. Tuning: dropped bare `\bpaid\b` from `FINANCE_KEYWORDS` in favor of more specific phrases (`payment processed`, `amount paid`, `successful payment`, `card was charged`, `your invoice`, `your receipt`, `billing statement`, `renewal receipt`, `thank you for your payment`, `auto-?pay`); expanded `FINANCE_DOMAINS` (~3x) with the missing Hawaii utility/govt/bank set + a broad SaaS/payroll/wholesale set. Re-scored the same sample: **precision ≈ 0.94, recall ≈ 0.92**. Acceptable for Stage-1 (Stage-2 AI extraction still gates on `confidence ≥ 0.5`, so residual FPs cost one Anthropic call but never silently create an invoice).
- **Stage 2 — AI extraction** (`POST /api/finance/extract-from-message`, worker-key gated): loads the message + first PDF attachment from R2, base64-encodes the PDF in 32KB chunks, sends both as document+text content blocks to Anthropic `claude-sonnet-4-20250514`, and parses a strict JSON response (`kind`, `vendor`, `amount_cents`, `invoice_number`, `issue_date`, `due_date`, `paid`, `confidence`, `summary`). Idempotent on `source_message_id` so retries never double-import. Caches the raw extraction on `email_messages.finance_extracted_json` regardless of outcome.
- **Stage 3 — auto-create with review badge**: extractions with `kind != "not_finance"`, `confidence >= 0.5`, and `amount > 0` insert directly into `invoices` with `auto_imported=1`, vendor as client, file_url linked to the source PDF. Receipts/`paid=true` extractions land as `status='paid'`; bills/statements as `status='pending'` with the parsed `due_date`. Note field includes the AI summary + source message ID for audit.
- **Finances tab review UI** (`InvoicesTab` in `client/src/pages/ai-config.tsx`): rows with `auto_imported=1` get an amber background, an "Auto · Review" pill (with confidence % in tooltip), and dedicated **Approve** (POST `/api/invoices/:id/approve-auto` clears the flag) / **Reject** (deletes via existing DELETE) / **Edit** buttons. Manual rows render the original Mark-Paid / Edit / Delete trio.
- **Production setup**: D1 — `npx wrangler d1 execute savvy-admin --remote --file=migrations/0027_auto_finance.sql`. Pages — needs `ANTHROPIC_API_KEY` + `WORKER_KEY` + `FILES_BUCKET` (R2) bindings, all already present from prior bundles. Worker — needs `WORKER_KEY` + `INTERNAL_API_URL` (already set in `wrangler.toml`). No new env vars introduced.
